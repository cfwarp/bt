...
select_node(){
    ...
    if [ ! -f $public_file ];then
        download_file $public_file http://v7.hostcli.com/install/public.sh
    fi
    ...
    downloads_Url=https://v7.hostcli.com
}

get_version(){
    version=$(curl -Ss --connect-timeout 5 -m 2 https://seele.wang/api/panel/get_version)
    if [ "$version" = '' ];then
        version='7.6.0'
    fi
}
